...
public:
    ...
    // Get u (duty cycle) buffer contents (returns number of valid elements)
    uint16_t getUbuffer(float* uOut);

    // Get y (lux) buffer contents (returns number of valid elements)
    uint16_t getYbuffer(float* yOut);
