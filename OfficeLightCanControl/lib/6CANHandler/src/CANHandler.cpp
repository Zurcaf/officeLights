      ...
      m_counter(0)
{
    ...
    m_node_address = m_board_id.id[7]; // Using last byte as node address
}
...
bool CANHandler::sendMessage(uint32_t id, const uint8_t *data, uint8_t length) {
...
bool CANHandler::readMessage(uint32_t *id, uint8_t *data, uint8_t *length) {
    struct can_frame frame;
